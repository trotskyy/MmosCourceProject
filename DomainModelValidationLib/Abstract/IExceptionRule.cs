...
namespace DomainModelValidation.Abstract
{
    ...
    public interface IExceptionRule<out TDbEntity, TUnitOfWork> : IValidationRule<TDbEntity, TUnitOfWork>
        ...
        where TUnitOfWork : class
    {
        ...
        IExceptionRule<TDbEntity, TUnitOfWork> Including(Func<TDbEntity, TUnitOfWork, bool> validationRulePredicate);

        /// <summary>
        /// Define exception which will be thrown in case of validation rules violation. Default exception if not set
        /// </summary>
        IExceptionRule<TDbEntity, TUnitOfWork> ThrowingException(Exception exception);
    }
}
